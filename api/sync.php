...
try {
    if ($method === 'GET') {
        if ($action === 'get_all') {
            ...
            try {
                ...
                $conn->exec("CREATE TABLE IF NOT EXISTS rm_units (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(50) UNIQUE, conversion_rate DECIMAL(15,6) DEFAULT 1.0)");
            ...
            $data = [
                ...
                'rmUnits' => $conn->query("SELECT id, name, conversion_rate AS rate FROM rm_units")->fetchAll(PDO::FETCH_ASSOC),
    ...
    elseif ($method === 'POST') {
        ...
        elseif ($action === 'save_rm_unit') {
            $u = $input['unit'];
            $stmt = $conn->prepare("INSERT INTO rm_units (name, conversion_rate) VALUES (?, ?) ON DUPLICATE KEY UPDATE conversion_rate=VALUES(conversion_rate)");
            $stmt->execute([$u['name'], $u['rate'] ?? 1.0]);
            echo json_encode(['status' => 'success']);
        }
